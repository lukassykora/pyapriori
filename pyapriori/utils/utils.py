...
def get_support(data: Data, numpy_or_cupy: ModuleType, counts: MultiDimensionalArray = None) -> MultiDimensionalArray:
    """Get support
...
def generate_candidates(
    ...
    previous_multiplier_mask: List[int] = None, t1=0, t2=0, t3=0
    ...
    return multiplier_mask_left, multiplier_mask_right, t1, t2, t3


def itemsets_support(
    data: Data, multiplier_mask_left: List[int], multiplier_mask_right: List[int], counts: MultiDimensionalArray, t4, t5
    ...
    if isinstance(data, (csr_matrix, csc_matrix, cupy_csr_matrix)):
        ...
        data = data[:, multiplier_mask_left].minimum(data[:, multiplier_mask_right])
    else:
    ...
    # Unique rows
    # sh0 = data.shape[0]
    # data, inverse = np.unique(data, axis=0, return_inverse=True)
    ...
    start = time.time()
    data_support = get_support(data, get_numpy_or_cupy(data), counts)
    end = time.time()
    t5 += end - start
    ...
    return data, data_support, counts, t4, t5
...
def min_support_set(
    ...
    return candidates, candidates_support, multiplier_mask_left, data, counts
